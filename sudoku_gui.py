...
def generate():
    ...
    for i in range(0, 9, 3):
        input_num = list(range(1, 10))
        random.shuffle(input_num)
        for x in range(3):
            for y in range(3):
                board[i + x][i + y] = input_num.pop()
    def non_diagonal(board, x, y):
        if x == 9:
            return True
        if y == 9:
            return non_diagonal(board, x + 1, 0)
        if board[x][y] != 0:
            return non_diagonal(board, x, y + 1)
        for test in range(1, 10):
            if valid(board, (x, y), test):
                board[x][y] = test
                if non_diagonal(board, x, y + 1):
                    return True
        board[x][y] = 0
        return False
    non_diagonal(board, 0, 0)
    for _ in range(random.randint(63,64)):
...
class Board:
    ...
    def redraw(self, keys, wrong, time):
        ...
        if wrong > 0:
            ...
            font = pygame.font.SysFont('arial', 40) #Number of Incorrect Inputs
            text = font.render(str(wrong), True, (0, 0, 0))
            self.window.blit(text, (32, 542))

        font = pygame.font.SysFont('arial', 40) #Time Display
    ...
    def hint(self, keys):
        ...
        while self.board!= self.solvedBoard:
            x=random.randint(0,8)
            y=random.randint(0,8)
            if self.board[x][y] == 0 and (x,y) not in keys:
                num=self.solvedBoard[x][y]
                self.board[x][y]=num
                self.tiles[x][y].value=num
                break
...
def main():
    ...
    pygame.display.set_caption("SusSud")
